...
internal unsigned char SendPidMap(int outChnId)
{
	bool rslt = true;
	...
	if (outChnId <= outPrgListLen)
	{
		for (int i = 0; i < outPrgListLen; i++ )
		{
			if (outChnId == 0 || i + 1 == outChnId)
			{
				...
				list_get(&prginfolist, i, &eachChn);
				
				if(eachChn){
					iSelectedCnt += list_len(&eachChn->prgNodes);				
					iSelectedCnt += eachChn.userPrgNodes.Count;					
				}
...
{
	unsigned char isNeedDesInfoSend = true;
	int selCnt = CountSelectedPrgCnt(outChnId);
	if (selCnt > MuxPrgNumMax)
	{		
		isNeedDesInfoSend = false;
		return;
	}
    unsigned char sendRslt = clsProgram.AutoMux_makeMuxInfoAndSend(outChnId, !db.valueTree.outChnArray[outChnId - 1].isManualMapMode);
    if (!sendRslt)
    {	    
	    isNeedDesInfoSend = false;
	    return;
	...
	{
		clsProgram.MakeOutputBytesAndSend(outChnId);
	}
