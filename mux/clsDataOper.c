...
static int LittleFormat_intToBytes(int intValue,  unsigned char outBytes[], int offset, int bytesLen)
{
	...
	{
		outBytes[offset++] = (unsigned char)((intValue) & (0xFF));
		intValue >>= 8;
	}
...
static unsigned short LittleFormat_pidFromBytes(int offset, unsigned char inBytes[])
{
	return (unsigned short)(((inBytes[offset + 1] & 0x1f) << 8) | inBytes[offset]);
}

static int LittleFormat_pidToBytes(int intValue, unsigned char outBytes[], int offset)
{
	outBytes[offset + 1] = (unsigned char)((intValue >> 8) & 0x1f);
	outBytes[offset] = (unsigned char)intValue;
	return 2;
}
...
// static unsigned char GetIntFromHex(string hexStr, out int outIntNum)
// {
// 	outIntNum = 0;
...
// static unsigned char HexStringBySpaceToBytes(string hexStr, out unsigned char outBytes[])
// {
// 	outBytes = null;
...
static unsigned short BigFormat_pidFromBytes(int offset, unsigned char inBytes[])
{
	return (unsigned short)(((inBytes[offset] & 0x1f) << 8) | inBytes[offset + 1]);
}
